export default async function uuidToBasicUser(uuid: string): Promise<BasicMinecraftUser> {

    const mojangApi = "https://sessionserver.mojang.com/session/minecraft/profile/";


    let user: BasicMinecraftUser = {
        uuid: uuid,
        name: "Unknown",
        // Default Steve texture:
        texture: ""
    }

    const res = await fetch(mojangApi + uuid);
    if (res.ok) {
        const json = await res.json();

        const name = json.name;
        if (name) user.name = name;

        const properties = json.properties;
        if (properties) {
            const texture = properties.find((p: { name: string }) => p.name == "textures");
            if (texture) user.texture = JSON.parse(atob(texture.value)).textures.SKIN.url;
        }
    }

    return user;
}